...
B = B_sub = 18               # Block size
M = 216               # Subsample size
# B_sub = 18           # Subsample block size
...
run_simulation_blockwise = function(n_sim){
  ...
  for (n in 1:n_sim) {
    ...
    # Subsampling for F_l
    L = T - M + 1
    F_l_values = sapply(1:L, function(l) {
      X_sub = X[l:(l+M-1), ]
      max_error = 0
      
      for (k in 1:num_thetas_sub) {
        F_l_theta_hat = spectral_density_estimator(Theta_sub[k], X_sub, B_sub)
        F_theta_hat = F_theta_hat_list[[ratio * k]]
        error = F_l_theta_hat - F_theta_hat
        max_error = max(max_error, max(abs(Re(error))), max(abs(Im(error))))
      }
      
      sqrt(M/B_sub) * max_error
    })
    
    F_l_quantile_95 = quantile(F_l_values, 0.95)
    F_l_quantile_90 = quantile(F_l_values, 0.9)
    
    # Store F_l quantile
    F_l_quantiles_95[n] = F_l_quantile_95
    F_l_quantiles_90[n] = F_l_quantile_90
  ...
  return(list(# Z_hat_quantiles_95 = Z_hat_quantiles_95, Z_hat_quantiles_90 = Z_hat_quantiles_90, 
    F_l_quantiles_95 = F_l_quantiles_95, F_l_quantiles_90 = F_l_quantiles_90,
    F_star_values = F_star_values))
}

# Run the simulation with blockwise generation
simulation_results = run_simulation_blockwise(n_sim)

# F_star_Z_hat_95 = sum(simulation_results$F_star_values < simulation_results$Z_hat_quantiles_95)
# F_star_Z_hat_90 = sum(simulation_results$F_star_values < simulation_results$Z_hat_quantiles_90)
F_star_F_l_95 = sum(simulation_results$F_star_values < simulation_results$F_l_quantiles_95)
F_star_F_l_90 = sum(simulation_results$F_star_values < simulation_results$F_l_quantiles_90)


print(paste("p =", p, "T =", T, "B =", B, "M =", M))
# print(paste("F_star vs Z_hat 95% quantile: " , F_star_Z_hat_95))
# print(paste("F_star vs Z_hat 90% quantile: " , F_star_Z_hat_90))
print(paste("F_star vs F_l 95% quantile: ", F_star_F_l_95))
print(paste("F_star vs F_l 90% quantile: ", F_star_F_l_90))
